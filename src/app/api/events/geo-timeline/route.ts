...
export async function GET(req: NextRequest) {
  ...
  const data = await cached(`org:${orgId}:geotl:${hours}:${buckets}`, 30, async () => {
    const rows = await db.execute<{
      bucket: string; lat: number; lon: number; country: string; city: string; count: number; threats: number;
    }>(sql`
      SELECT
        to_char(
        ...
        e.geo_lat as lat, e.geo_lon as lon, e.geo_country as country,
    ...
    return { hours, buckets: sorted.length, frames: sorted.map((k) => ({ time: k, points: frames[k] })) };
  });
